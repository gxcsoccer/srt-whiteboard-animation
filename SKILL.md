...
| 开场封面 | 默认必做：Codex 出封面分镜（上半留白、图内无字）+ 渲染器手写主标/副标，片头 4–6 秒后擦入第一幕；`--no-cover` 跳过。 |
| 文字区 | 每幕一行标题 + 2–4 条要点，`type: "text"` 元素，由渲染器手写；除此之外画面禁止任何文字。 |
| 幕间过渡 | 合并时每个接缝插入「完整画面停留 ≥500ms + 橡皮擦除」，擦除目标是下一幕**已有墨**的一帧（自动裁掉片头空白），禁止闪回空白画布。 |
...
每条成片都要有开场封面，除非用户明确说不要（脚本侧用 `--no-cover` 跳过）。做法与普通幕完全一致，
只是编排固定，由 `scripts/make_cover.py` 生成标注：

1. **主标 + 副标从 SRT/题目提炼**：主标 ≤8 字（概念本身），副标 ≤22 字（一句话说清它解决什么）。
2. **封面分镜**用 Codex 出图，和普通幕同一套小黑规范，但只画**一个** vignette 放在下半部分，
   **上半 40%–46% 必须是干净纸面**（标题要写在那里）；图内同样不许有任何文字。
3. **中文标题由渲染器手写**，绝不烤进出图：`make_cover.py` 会把标题区做成**通栏**（约画布 90% 宽）。
4. **片头节奏 4–6 秒**：先写主标、再写副标、然后点缀小黑，末尾留 0.4s 凝视；
   合并时 `merge_scenes.py` 的 hold 会把停留凑到约 1 秒，然后擦入第一幕。

```bash
python scripts/make_cover.py --board 封面.png --title "动态组合" \
    --subtitle "把可撤销效应和响应式协效应做成运行时" \
    --output 封面.annotation.json          # 加 --no-cover 就整步跳过
```

封面渲染与普通幕同一条命令（`render_stream_whiteboard.py`），合并时用 `--cover 封面.mp4` 放到最前。
...
6. **命令行渲染成片。** 仅在用户确认最终标注与时序后，用 `render_stream_whiteboard.py` 逐幕出全清 MP4（**封面也用同一条命令渲染**），抽查开场、任意重叠模块中段、结尾三个时间点。**完成后停止，等待用户确认成片。**
7. **多幕合并（仅适用于多幕）。** 仅在用户确认所有单幕成片后，用 `merge_scenes.py` 按顺序合并成一条，并用 `--cover 封面.mp4` 把封面接在最前面（不要封面时用 `--no-cover`）。**幕与幕之间必须有过渡，禁止硬切回空白画布**：默认在每个接缝插入「上一幕完整画面停留 600ms（下限 500ms）+ 橡皮擦除 700ms」，擦除终点是下一幕**已经有墨的第一帧**——合并时会自动裁掉每幕片头那段还没落笔的空白纸（`--seam-ink-ratio` 控制"算有墨"的门槛），所以擦完不会闪回白板。同时用 `--timeline-out` 导出时间线，供下一步重定时字幕。**完成后停止，等待用户确认最终合成视频。**
8. **重定时字幕 + 配旁白（edge-tts 混音）。** 仅在用户确认最终合成视频（单幕项目则是确认该幕成片）后：
   - 先用 `retime_srt.py` 把字幕对齐到成片的真实时间线：`--timeline` 用第 7 步导出的 JSON（补上过渡造成的位移），`--annotations` 给各幕标注（把每条字幕对齐到它对应区域的**实际作画窗口**，先落笔、再开口）。每幕第一条字幕会随标题书写就开口，避免幕首静音；`--no-text-lead` 可关掉。
...
6. **多幕合并（封面 + 幕间过渡）**：
   ```bash
   <ENV_PY> scripts/merge_scenes.py --inputs 幕1.mp4 幕2.mp4 幕3.mp4 \
       --cover 封面.mp4 --output final.mp4 \
       [--hold-ms 600] [--erase-ms 700] [--seam-ink-ratio 0.0045] --timeline-out timeline.json
   ```
   `--cover` 把封面接在最前（`--no-cover` 跳过）。
   默认每个接缝 = 上一幕完整画面停留 600ms + 橡皮擦除 700ms（`--hold-ms 0 --erase-ms 0` 可关掉，
   但那样就是硬切回空白，不要用）。擦除目标是下一幕第一帧墨量达到 `--seam-ink-ratio` 的画面，
   之前的空白纸会被裁掉并记进 `leadTrimMs`。`--timeline-out` 记录封面长度、每幕起点与
   `leadTrimMs`，下一步重定时要用——不用它旁白就会和画面错开。
7. **重定时字幕**（插过渡后必须做，否则旁白比画面早开口）：
   ...
   不给则只按合并时间线整幕平移。每幕**第一条**字幕默认提前到文字区开始书写之后
   （写标题也是在动笔，不必让幕首静音 5–6 秒；它讲的就是本幕主旨，与标题同义），
   想回到"等标题写完再开口"就加 `--no-text-lead`。
...
- 片头有封面：主标/副标是手写出来的（不是烤进图里）、通栏够大，写完点缀小黑，停留约 1 秒后擦入第一幕。
- 每幕都有小黑，且小黑承担核心动作（不是站在旁边看）。
- 每幕都有手写标题 + 2–4 条要点，字形清晰、没写出区域、与隐喻不重叠；画面里没有别的文字。
- 幕与幕的接缝：上一幕完整画面停留 ≥0.5s，然后擦除过渡；擦完露出的是下一幕**已经有墨**的画面，**没有**闪回空白纸。

...
- **可分区（白板管线特有）：** 每个要单独出场的主体必须能被一个**互不相交**的矩形框住，主体之间留出明显的空白走廊；主体数量与该幕的字幕事件数一致（通常 2–4 个），按叙事顺序从左到右排布；四角保持干净纸底（渲染器靠四角取样识别背景色）。
- **绝对禁止：** 画面内的任何文字、词语、字母、数字、字体、标签、标题或手写批注（**上游要求的中文手写批注在本仓库被禁用**，解释职责全部交给字幕）；写实感、摄影细节、3D 效果、绘画质感；PPT 信息图、正式流程图、课件、可爱卡通、儿童插画、复杂架构图、真实 UI 截图；复杂背景、密集装饰、高饱和度画面。
...
- 每幕都有小黑，且小黑承担核心动作（不是站在旁边看）；画面内没有任何文字。

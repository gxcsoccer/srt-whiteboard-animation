...
| 幕间过渡 | 合并时每个接缝插入「完整画面停留 ≥500ms + 橡皮擦除」，擦除目标是下一幕**已有墨**且**已抹掉它自己的手**的一帧（自动裁掉片头空白），禁止闪回空白画布、也不该同时出现两只手。 |
...
7. **多幕合并（仅适用于多幕）。** 仅在用户确认所有单幕成片后，用 `merge_scenes.py` 按顺序合并成一条，并用 `--cover 封面.mp4` 把封面接在最前面（不要封面时用 `--no-cover`）。**幕与幕之间必须有过渡，禁止硬切回空白画布**：默认在每个接缝插入「上一幕完整画面停留 600ms（下限 500ms）+ 橡皮擦除 700ms」，擦除终点是下一幕**已经有墨的第一帧**，并且会把那一帧里下一幕自己的手抹掉——擦场时画面上只该有擦的那只手，下一幕的手等擦完再进。合并时自动裁掉每幕片头还没落笔的空白纸（`--seam-ink-ratio` 控制"算有墨"的门槛，默认很低，只裁空白、几乎不裁作画），裁掉多少记进 `leadTrimMs` 由下一步补回。同时用 `--timeline-out` 导出时间线，供下一步重定时字幕。**完成后停止，等待用户确认最终合成视频。**
...
6. **多幕合并（封面 + 幕间过渡）**：
   ...
   ```
   `--cover` 把封面接在最前（`--no-cover` 跳过）。擦场默认抹掉下一幕的手（`--keep-next-hand` 保留）。
...
- 擦场时画面上只有擦的那只手；下一幕的手在擦完之后才进入。
- 片头不静音：有封面时第一句在写主标期间就开口。

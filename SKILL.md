...
| 输出尺寸 | 默认长边上限 1080 像素；按需用 `--cap-long-edge` 调整。 |
...
2. **生成并检查线稿。** 仅在用户确认策略后，按“统一出图视觉规范”逐幕生成 16:9 暖米黄旧纸张底线稿图，背景 `#F5EBD7`，主体之间保留充足留白便于自动拆分；不得生成文字、复杂照片、重叠对象或与规范冲突的元素。逐张检查关键人物/物体是否可辨认、肢体和轮廓是否完整、叙事关系是否正确；结构错误时必须在本步重新生成，渲染器不能修复画错的对象。**完成后停止，展示线稿并等待用户确认。**
...
5. **在预览台调整并保存。** 仅在用户确认预览图后，在已打开且已载入对应目录的预览台调整：默认（未播放）显示完整图片和区域框；画布是**矩形代理**：拖区域四边四角改 `region`，右侧改名称/方向/**开始(ms)/结束(ms)**（时长= 结束−开始，只读）与**字幕**，拖动模块列表**调整顺序**（自动重排 `sequence`，并保留原开场延迟/元素间隔重新串行排时），选中模块自动高亮对应字幕；拖时间轴或按播放看揭示（未开始区域不显示）；`direction` 只影响此代理。改完点“保存本场景/全部保存”写回原 `.annotation.json`。保存前必须通过区域边界、sequence 连续性与时间不重叠校验，并把 `sceneDurationMs` 对齐到最后区域结束+0.5s。**保存后停止，等待用户确认最终标注与时序。**
6. **命令行渲染成片。** 仅在用户确认最终标注与时序后，用 `render_stream_whiteboard.py` 逐幕输出 MP4 画面轨（默认长边 1080），抽查开场、任意重叠模块中段、结尾三个时间点。**完成后停止，等待用户确认成片。**
...
- **区域串行作画：** stream 画法是一支笔在动，同一幕内各区域必须**在时间上依次进行**（`startMs` 不重叠）：下一区域从上一区域 `startMs + durationMs`（+ 可选 100–300ms 呼吸）开始。预览台保存和命令行渲染都会拒绝重叠时序。
...
需要 Python 3.11+；直接依赖版本固定在仓库根目录 `requirements.txt`。

1. **准备环境**（首次或缺依赖时）：
   ```bash
   python scripts/prepare_env.py --check   # 探测；成功末行输出 ENV_PY=<路径>，捕获备用
   python scripts/prepare_env.py           # 缺则建 .venv 并按 requirements.txt 安装固定依赖
   ```
2. **解析字幕 + 建议分镜**：
   ```bash
...
5. **渲染单幕成片**：
   ...
   <ENV_PY> scripts/render_stream_whiteboard.py <图片> <标注> <输出mp4> assets/drawing-hand.png \
       [--ink-path grid|skeleton] [--color-fill contour-wipe|brush] \
       [--total-ms <毫秒>] [--cap-long-edge <像素>]
   ```
...
> 输出 MP4 仅含白板动画画面，不自动混入配音、背景音乐或烧录 SRT 字幕；这些属于后续视频合成步骤。
